# Behaviourism Glossary

Fuzzy-search the 258-term Behaviour Analysis Glossary from [animallearning.org](https://animallearning.org/Clippings/articles/behaviour-glossary) — radical-behaviourism vocabulary at your launcher.

<!-- (screenshot pending first launch) -->

## Why this exists

I stumbled upon [the source glossary](https://animallearning.org/Clippings/articles/behaviour-glossary) while learning about behaviourism and found myself going back to it all the time to look terms up, hence the Raycast/Vicinae extension.

The glossary itself is © [animallearning.org](https://animallearning.org/), maintained by **Juju & Ana**. This extension reproduces it verbatim for offline fuzzy-search — if the data is useful to you, go support the source.

## Features

- 258 behaviour-analysis terms, definitions copied verbatim from the source
- Fuzzy search across terms, acronyms (DRO, DRA, FR, VR, FI, VI, CRF, MTS…), and categories
- Detail pane with the full definition rendered as markdown
- Light-touch category grouping (Schedules, Verbal Behavior, Stimulus Control, Reinforcement & Punishment, …)
- One-keystroke actions: copy definition, copy term, open source page
- Fully local — no network calls, no telemetry

## Install

### Vicinae (Linux)

Requires [Vicinae](https://vicinae.com) and the `vici` CLI on `PATH`.

```sh
git clone https://github.com/arch-err/behaviourism-glossary.git
cd behaviourism-glossary
npm install
npm run dev
```

`npm run dev` registers the extension with the running Vicinae daemon and hot-reloads on file changes. For a packaged artifact use `npm run build`.

### Raycast (macOS)

The `@vicinae/api` package mirrors `@raycast/api` closely, so the source is mostly portable:

1. Replace `@vicinae/api` with `@raycast/api` in `package.json`
2. Update the import in `src/behaviourism-glossary.tsx` to `@raycast/api`
3. Drop the `$schema` and `vici`-specific scripts in favour of `ray develop`

Not currently shipped as a Raycast variant — patch locally if you need it.

## Usage

Open your launcher, type "behaviour" (or your fuzzy-match of choice), pick the **Search Glossary** command. Inside:

- Type a term: `differential reinforcement`, `mand`, `escape`
- Type an acronym: `DRO`, `FI`, `MTS`, `CRF`, `EAB`
- Type a category: `schedules`, `verbal`, `aversive`

`Enter` copies the definition. The secondary action copies the term. Tertiary action opens the source page on animallearning.org.

## Development

```sh
npm install
npm run dev      # live in Vicinae
npm run lint     # vici lint
npm run format   # biome format --write src
npm run build    # vici build
```

Source layout:

```
src/
  behaviourism-glossary.tsx   # the List view
  glossary.ts                  # the 258-entry data file
```

## Acknowledgements

- [animallearning.org](https://animallearning.org/) — source glossary, maintained by Juju & Ana
- [Vicinae](https://vicinae.com) — Raycast-compatible launcher on Linux
- [Raycast](https://raycast.com) — the API surface this extension targets

## License

MIT — see [LICENSE](LICENSE). Glossary content is © animallearning.org and is reproduced here with attribution for offline reference.
